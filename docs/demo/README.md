...
`dee168be929007ddab86ef7dff7fb27d0d8c99d6` and to a SHA-256 for every file.
...
- C — `./demo C` — The current report separates eligible Coverage cells and pair obligations into BLOCKED, UNCOVERED, and excluded; it also exposes that committed evidence does not support the requested 43-Scenario claim.
...
- F — `./demo F` — The committed fail-closed completion check is the honest historical roadmap, including its now-stale ledger gap.
...
## Coverage-cardinality caveat

The task asks for a 43-Scenario pairwise target, but no committed artifact says
43. The current report records 4,368 eligible Coverage cells and 374 eligible
pairs (326 UNCOVERED and 48 BLOCKED), plus 32 excluded pairs. ADR 0007 records
an older 42-Scenario greedy estimate for a different snapshot. Step C reports
this discrepancy rather than turning an unsupported assumption into evidence.
