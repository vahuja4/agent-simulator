...
- The expected baseline is recorded in `ENVIRONMENT.md`. Run the full baseline
  ...
  imposes a narrower read boundary. Do not edit any tracked file, data files
  included, while that run is still in progress: tests read them mid-run. A commit that changes the offline suite's
